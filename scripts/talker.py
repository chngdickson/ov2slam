...
class SyncDepth(CompatibleNode):
    def __init__(self):
        self.role_name = "ego_vehicle"

        self.rgb_count = 0 # TODO: temp as debug
        rospy.Subscriber("/carla/{}/rgb_front/image".format(self.role_name), Image, self.rgb_callback)

        sub_list_depth = [(Image, 'rgbd_front/image'), (Image, 'rgbd_front_right/image'), (Image, 'rgbd_front_left/image'),
                            (Image, 'rgbd_back/image'), (Image, 'rgbd_back_right/image'), (Image, 'rgbd_back_left/image'),
                        ]
        self.depth_count = 0 # TODO: temp as debug
        self.depth_sensors(sub_list_depth)

    def depth_sensors(self, sub_list_depth):
        subbed_list = []
        # depth_count = 0 # TODO: temp as debug
        for sub in sub_list_depth:
            subbed_list.append(message_filters.Subscriber("/carla/{}/{}".format(self.role_name, sub[1]), sub[0]))

        # ts = message_filters.ApproximateTimeSynchronizer(subbed_list, 1, 0.005)
        ts = message_filters.TimeSynchronizer(subbed_list, 1)

        ts.registerCallback(self.depth_callback)

    def depth_callback(self, *args):
        print(f'\nfsec: {args[0].header.stamp.secs} {args[0].header.stamp.nsecs}')
        for arg in args:
            print(f'frame_id: {arg.header.frame_id}')
        self.depth_count += 1
        print(f'depth_sync_count: {self.depth_count}')

    def rgb_callback(self, msg):
        self.rgb_count += 1
        print(f'rgb_count: {self.rgb_count}')


## This is init
def main(args=None):
    roscomp.init('listener', args=args)

    executor = roscomp.executors.MultiThreadedExecutor()
    try:
        sync_depth = SyncDepth()
        executor.add_node(sync_depth)

        roscomp.on_shutdown(sync_depth.destroy)
